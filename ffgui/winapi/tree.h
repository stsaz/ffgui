...
typedef ffui_view ffui_tree;

FF_EXTERN int ffui_tree_create(ffui_tree *t, ffui_window *parent);
...
static inline void* ffui_tree_ins(ffui_tree *t, void *parent, void *after, ffui_tvitem *it) {
	TVINSERTSTRUCTW ins = {};
	ins.hParent = (HTREEITEM)parent;
	ins.hInsertAfter = (HTREEITEM)after;
	ins.item = it->ti;
	void *r = (void*)ffui_send(t->h, TVM_INSERTITEMW, 0, &ins);
	ffui_tree_reset(it);
	return r;
}
#define ffui_tree_append(t, parent, it)  ffui_tree_ins(t, parent, FFUI_TREE_LAST, it)

#define ffui_tree_remove(t, item)  ffui_ctl_send(t, TVM_DELETEITEM, 0, item)

static inline void ffui_tree_get(ffui_tree *t, HTREEITEM pitem, ffui_tvitem *it) {
	it->ti.hItem = pitem;
	ffui_send(t->h, TVM_GETITEMW, 0, &it->ti);
}

static inline char* ffui_tree_text(ffui_tree *t, void *item) {
	wchar_t buf[255];
	TVITEMW it = {};
	it.mask = TVIF_TEXT;
	it.pszText = buf;
	...
	if (ffui_ctl_send(t, TVM_GETITEMW, 0, &it))
